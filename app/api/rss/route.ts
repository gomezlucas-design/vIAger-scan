...
function isZoneAutorisee(cp?: string): boolean {
  if (!cp) return true // Si pas de CP, on garde (sera filtré après enrichissement)
...
// ─── Sources avec URLs géo-filtrées ──────────────────────────────────────
const SOURCES = [
  // Renée Costes — pages régionales
...
export async function GET(req: NextRequest) {
  ...
  const runBackground = async () => {
    ...
    console.log(`RSS: ${allListings.length} trouvées, ${filtered.length} après filtres`)

    // Sauvegarder via /api/import
  ...
  return NextResponse.json({
    success: true,
    message: `Scraping ${SOURCES.length} sites viager lancé`,
