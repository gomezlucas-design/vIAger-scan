...
const C = {
  bg:"#0D0D0D",surface:"#141414",card:"#1A1A1A",border:"#2A2A2A",border2:"#333333",
  orange:"#FF6B35",orange2:"#FF8C5A",orangeD:"#CC4A1A",gold:"#F5A623",blue:"#4A9EFF",
  green:"#34D399",red:"#F87171",yellow:"#FCD34D",text:"#F0F0F0",text2:"#A0A0A0",text3:"#606060",white:"#FFFFFF",
};
...
const getEsp = (age: number, sex: string): number => (INSEE[sex]||INSEE.F)[Math.min(Math.max(age,60),90)]??10;

const ZONES: any[] = [
  [/paris\s*[12]/i,28],[/paris\s*[34]/i,27],[/paris\s*[56]/i,30],[/paris\s*[78]/i,29],
  [/paris\s*9(?!\d)/i,26],[/paris\s*1[0-9]/i,24],[/paris/i,25],[/neuilly|levallois/i,23],
  [/boulogne|issy|vanves/i,20],[/courbevoie|puteaux|montrouge|malakoff/i,18],[/marseille/i,13],
  [/lyon/i,16],[/nice|cannes|antibes/i,18],[/bordeaux/i,15],[/toulouse/i,13],[/aix/i,14],
  [/toulon/i,12],[/frejus/i,11],[/ciotat/i,12],[/cagnes/i,13],[/montpellier/i,13],
];
...
function computeIRR(flows: number[], guess=0.08): number {
  ...
  for (let i=0;i<80;i++) {
    let npv=0,deriv=0;
    for (let t=0;t<flows.length;t++) { npv+=flows[t]/Math.pow(1+rate,t); deriv+=-t*flows[t]/Math.pow(1+rate,t+1); }
...
function computeViager(o: any, libAns: number|null=null) {
  ...
  const coutMensuelOcc=dureeOccMois>0?((o.bouquet||0)/dureeOccMois)+(o.rente||0)+(tf+chargesAnn*ventil)/12:0;
  const dureeLibMois=libActive?(duree-libAns!)*12:0;
  const coutMensuelLib=dureeLibMois>0?((o.rente||0)*(1+majRente))+(tf+chargesAnn)/12-loyer:null;
  ...
  const hasPriceDrop=(o.priceHistory||[]).length>1&&o.bouquet<(o.priceHistory||[])[0]?.bouquet;
  const ratioVal=vv>0?prixNet/vv:null;
  let negoScore=0;
  if (ageJours>60) negoScore+=2; else if (ageJours>30) negoScore+=1;
  ...
  return {
    duree,libActive,totalRentes:rentes,tfTotal:tfTot,chgTotal:chg,loyers,loyerMensuelEffectif:loyer,
    prixBrut,prixNet,ratio:vv>0?prixNet/vv:null,decote:vv>0?(vv-prixNet)/vv:null,
    anneesOcc:libActive?libAns:duree,anneesLib:libActive?duree-libAns!:0,
    tri:computeIRR(flows)*100,van:computeNPV(flows,0.04),flows,
    coutMensuelOcc,coutMensuelLib,equilibreAtteint,chargesMensuellesLib,
...
function Badge({children,color=C.text3,bg}: any) {
  return <span style={{background:bg||`${color}18`,color,border:`1px solid ${color}30`,borderRadius:6,padding:"2px 8px",fontSize:10,fontWeight:700}}>{children}</span>;
}
function ProgressBar({value,max,color=C.orange}: any) {
  const pct=Math.min(100,Math.round((value/max)*100));
  return <div style={{background:C.border,borderRadius:3,height:3,overflow:"hidden"}}><div style={{background:color,width:`${pct}%`,height:"100%",borderRadius:3,transition:"width .4s ease"}}/></div>;
}
...
function DetailPanel({offre,onClose}: {offre: any;onClose: ()=>void}) {
  ...
  const TABS=[{id:"analyse",label:"Analyse"},{id:"cashflow",label:"Cash Flow"},{id:"liberation",label:"Libération"},{id:"historique",label:"Historique"},{id:"bien",label:"Bien"}];

  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.92)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:300}} onClick={onClose}>
      <div style={{background:C.surface,borderRadius:"20px 20px 0 0",width:"100%",maxWidth:680,maxHeight:"92vh",overflowY:"auto",border:`1px solid ${C.border}`,borderBottom:"none"}} onClick={e=>e.stopPropagation()}>
        <div style={{display:"flex",justifyContent:"center",padding:"12px 0 0"}}><div style={{width:36,height:4,background:C.border2,borderRadius:2}}/></div>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
            <div style={{flex:1}}>
              ...
              <div style={{fontSize:11,color:C.text3}}>
                ...
                {resBase.ageJours>0&&<span> ({fmtAge(resBase.ageJours)})</span>}
              </div>
            </div>
        ...
        <div style={{padding:20}}>
          {tab==="analyse"&&(
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10}}>
                {[{label:"Bouquet",value:fmt(offre.bouquet),sub:"Capital initial",color:C.text},{label:"Rente",value:fmt(offre.rente)+"/m",sub:"Mensuelle indexée",color:C.text},{label:"Durée",value:fmtYrs(res.duree),sub:"Espérance INSEE",color:C.yellow},{label:"Valeur vénale",value:fmt(offre.valeurVenale),sub:"Prix marché",color:C.text},{label:"Prix revient",value:fmt(res.prixNet),sub:"Total acquéreur",color:col},{label:"Décote",value:fmtPct(res.decote),sub:"Économie vs marché",color:res.decote>0?C.green:C.red}].map(k=>(
                  <div key={k.label} style={{background:C.card,borderRadius:10,padding:"12px 14px",border:`1px solid ${C.border}`}}>
                    <div style={{fontSize:9,color:C.text3,textTransform:"uppercase",marginBottom:5}}>{k.label}</div>
              ...
              <div style={{background:C.card,borderRadius:12,padding:16,border:`1px solid ${C.border}`}}>
                <div style={{fontSize:10,color:C.text3,textTransform:"uppercase",marginBottom:14}}>Décomposition du prix de revient</div>
                {[["Bouquet",offre.bouquet||0,C.orange],["Rentes cumulées",res.totalRentes,C.blue],["Taxe foncière",res.tfTotal,C.yellow],["Charges",res.chgTotal,C.text3]].map(([label,val,color])=>{
                ...
                <div style={{borderTop:`1px solid ${C.border}`,paddingTop:12,display:"flex",justifyContent:"space-between",fontSize:14,fontWeight:800}}>
                  <span style={{color:C.text2}}>Total</span><span style={{color:col}}>{fmt(res.prixNet)}</span>
                </div>
              </div>
              <div style={{background:C.card,borderRadius:10,padding:"12px 16px",border:`1px solid ${C.border}`,display:"flex",gap:20}}>
                <div><div style={{fontSize:9,color:C.text3,textTransform:"uppercase"}}>TRI</div><div style={{fontSize:14,fontWeight:700,color:C.blue}}>{fmtTRI(res.tri)}</div></div>
                <div><div style={{fontSize:9,color:C.text3,textTransform:"uppercase"}}>VAN (4%)</div><div style={{fontSize:14,fontWeight:700,color:res.van>=0?C.green:C.red}}>{fmt(res.van)}</div></div>
                <div style={{fontSize:10,color:C.text3,alignSelf:"center",flex:1}}>Métriques secondaires · basées sur récupération du bien au terme</div>
              </div>
            </div>
          )}
          ...
          {tab==="cashflow"&&(
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
              <div style={{background:C.card,borderRadius:12,padding:18,border:`1px solid ${C.border}`}}>
                <div style={{fontSize:10,color:C.text3,textTransform:"uppercase",marginBottom:14}}>Période occupée — Cash Flow mensuel</div>
                <div style={{fontSize:36,fontWeight:900,color:C.red,marginBottom:4}}>-{fmt(Math.round(resBase.coutMensuelOcc))}<span style={{fontSize:14,fontWeight:400,color:C.text3}}>/mois</span></div>
                <div style={{fontSize:11,color:C.text3,marginBottom:16}}>Effort financier mensuel moyen sur {fmtYrs(resBase.duree)}</div>
                <div style={{display:"flex",flexDirection:"column",gap:8}}>
                  {[["Amortissement bouquet",Math.round((offre.bouquet||0)/((resBase.duree||1)*12))],["Rente mensuelle",offre.rente||0],["Taxe foncière (÷12)",Math.round((offre.taxeFonciere||0)/12)],["Charges acquéreur (÷12)",Math.round(((offre.chargesCopro||0)*(offre.ventilationCharges||0.33))/12)]].map(([k,v])=>(
                    <div key={k as string} style={{display:"flex",justifyContent:"space-between",fontSize:12,padding:"8px 12px",background:C.surface,borderRadius:8}}>
                      <span style={{color:C.text2}}>{k}</span><span style={{color:C.red,fontWeight:600}}>-{fmt(v as number)}</span>
                    </div>
                  ))}
                </div>
              </div>
              <div style={{background:C.card,borderRadius:12,padding:18,border:`1px solid ${C.border}`}}>
                <div style={{fontSize:10,color:C.text3,textTransform:"uppercase",marginBottom:4}}>Après libération anticipée</div>
                ...
                {libEnabled&&resLib?(
                  <div>
                    <div style={{fontSize:32,fontWeight:900,color:resLib.coutMensuelLib!<0?C.green:C.red,marginBottom:4}}>
                      {resLib.coutMensuelLib!<0?"+":"-"}{fmt(Math.abs(Math.round(resLib.coutMensuelLib!)))}<span style={{fontSize:14,fontWeight:400,color:C.text3}}>/mois</span>
                    </div>
                    <div style={{fontSize:11,color:C.text3,marginBottom:14}}>{resLib.coutMensuelLib!<0?"🟢 Cash flow positif — le loyer couvre les charges":"🔴 Cash flow encore négatif"}</div>
                    <div style={{display:"flex",flexDirection:"column",gap:8}}>
                      {[["Rente majorée (+30%)",-Math.round((offre.rente||0)*1.3),C.red],["TF + charges (100%)",-Math.round(((offre.taxeFonciere||0)+(offre.chargesCopro||0))/12),C.red],["Loyer perçu",Math.round(resLib.loyerMensuelEffectif),C.green]].map(([k,v,c])=>(
                        <div key={k as string} style={{display:"flex",justifyContent:"space-between",fontSize:12,padding:"8px 12px",background:C.surface,borderRadius:8}}>
                          <span style={{color:C.text2}}>{k}</span><span style={{color:c as string,fontWeight:600}}>{(v as number)>0?"+":""}{fmt(v as number)}</span>
                        </div>
                      ))}
                    </div>
                    {resLib.equilibreAtteint&&<div style={{marginTop:12,background:`${C.green}15`,border:`1px solid ${C.green}30`,borderRadius:10,padding:"10px 14px",fontSize:11,color:C.green}}>✅ Le loyer ({fmt(resLib.loyerMensuelEffectif)}/m) couvre les charges ({fmt(Math.round(resLib.chargesMensuellesLib))}/m)</div>}
                  </div>
                ):(
                  <button onClick={()=>setTab("liberation")} style={{background:`${C.orange}15`,border:`1px solid ${C.orange}30`,color:C.orange,borderRadius:10,padding:"10px 16px",cursor:"pointer",fontSize:12,fontWeight:600,width:"100%"}}>Simuler une libération anticipée →</button>
                )}
              </div>
            </div>
          )}
            ...
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
              <div style={{background:C.card,borderRadius:12,padding:18,border:`1px solid ${C.border}`}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:libEnabled?18:0}}>
                  ...
                  <button onClick={()=>setLibEnabled(p=>!p)} style={{background:libEnabled?C.orange:C.card,color:libEnabled?C.white:C.text3,border:`1px solid ${libEnabled?C.orange:C.border2}`,borderRadius:20,padding:"7px 18px",cursor:"pointer",fontSize:12,fontWeight:700}}>
                ...
                {libEnabled&&(
                  <div>
                    <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:16}}>
                      <div style={{flex:1}}>
                        <input type="range" min={1} max={Math.max(1,Math.floor(dureeMax-0.5))} step={0.5} value={libAns} onChange={e=>setLibAns(+e.target.value)} style={{width:"100%",accentColor:C.orange}}/>
                        <div style={{display:"flex",justifyContent:"space-between",fontSize:10,color:C.text3,marginTop:4}}><span>An 1</span><span>Esp. vie {fmtYrs(dureeMax)}</span></div>
                      </div>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
                      {[["Majoration rente",`+${((offre.majorationRenteLiberation||0.30)*100).toFixed(0)}%`,C.yellow],["Charges","100% acquéreur",C.yellow],["Loyer estimé",`${fmt(resLib?.loyerMensuelEffectif||0)}/m`,C.green]].map(([k,v,c])=>(
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                {[{label:"Occupé à vie",result:resBase,color:scoreColor(resBase.ratio),active:true},{label:libEnabled?`Libéré an ${libAns}`:"Scénario libération",result:resLib||resBase,color:resLib?scoreColor(resLib.ratio):C.border2,active:!!resLib}].map(({label,result,color,active})=>(
                  <div key={label} style={{background:C.card,borderRadius:12,padding:16,border:`1px solid ${active?color+"40":C.border}`,opacity:active?1:0.4}}>
                    ...
                    {resLib&&label.includes("Libéré")&&<div style={{fontSize:11,marginTop:6,fontWeight:700,color:resLib.ratio<resBase.ratio?C.green:C.red}}>{resLib.ratio<resBase.ratio?"▲ Meilleur ratio":"▼ Ratio moins bon"}</div>}
                  </div>
                ))}
              </div>
            </div>
          ...
          {tab==="historique"&&(
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
              <div style={{background:C.card,borderRadius:12,padding:16,border:`1px solid ${C.border}`}}>
                <div style={{fontSize:10,color:C.text3,textTransform:"uppercase",marginBottom:16}}>Historique des prix</div>
                    ...
                    <div key={i} style={{display:"flex",gap:14,marginBottom:14,paddingBottom:14,borderBottom:!isLast?`1px solid ${C.border}`:"none"}}>
                      ...
                      <div style={{flex:1}}>
                        ...
                        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                          {[{label:"Bouquet",value:fmt(h.bouquet),diff:baisseB?`↓ ${fmt(prev.bouquet-h.bouquet)}`:null},{label:"Rente",value:`${fmt(h.rente)}/m`,diff:baisseR?`↓ ${fmt(prev.rente-h.rente)}/m`:null}].map(f=>(
              ...
              <div style={{background:`${C.orange}0D`,border:`1px solid ${C.orange}20`,borderRadius:12,padding:14,display:"flex",gap:12}}>
                ...
                <div style={{fontSize:11,color:C.text2,lineHeight:1.6}}>
                  <strong style={{color:C.orange}}>Signal de négociation :</strong> annonce de +60j avec baisse de prix = vendeur motivé. Score : <strong style={{color:resBase.negoColor}}>{resBase.negoLabel}</strong> ({resBase.ageJours}j, {resBase.hasPriceDrop?"prix baissé":"prix stable"})
                </div>
              </div>
            </div>
          )}
          ...
          {tab==="bien"&&(
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                {[["Superficie",offre.superficie?`${offre.superficie} m²`:"—"],["Valeur vénale",fmt(offre.valeurVenale)],["Taxe foncière",`${fmt(offre.taxeFonciere)}/an`],["Charges copro",`${fmt(offre.chargesCopro)}/an`],["Occupant 1",offre.occupant1Age?`${offre.occupant1Age} ans ${offre.occupant1Sexe==="F"?"♀":"♂"}`:"—"],["Occupant 2",offre.occupant2Age?`${offre.occupant2Age} ans ${offre.occupant2Sexe==="F"?"♀":"♂"}`:"—"],["Inflation",`${((offre.tauxInflation||0.03)*100).toFixed(1)}%`],["Croissance TF",`${((offre.tauxCroissanceTF||0.04)*100).toFixed(1)}%`]].map(([k,v])=>(
              ...
              {offre.note&&<div style={{background:C.card,borderRadius:10,padding:14,border:`1px solid ${C.border}`}}><div style={{fontSize:9,color:C.text3,textTransform:"uppercase",marginBottom:6}}>Notes</div><div style={{fontSize:12,color:C.text2,fontStyle:"italic",lineHeight:1.6}}>{offre.note}</div></div>}
              {offre.url&&<a href={offre.url} target="_blank" rel="noreferrer" style={{display:"block",background:C.orange,color:C.white,borderRadius:12,padding:"14px 20px",textAlign:"center",textDecoration:"none",fontSize:14,fontWeight:700}}>Voir l'annonce & contacter l'agence →</a>}
            </div>
          )}
        </div>
      </div>
...
function ImportModal({onClose,onImport}: {onClose: ()=>void;onImport: (o: any)=>void}) {
  ...
  const analyze=async()=>{
    ...
    try {
      ...
      setParsed(result);setEdited({...result.data,url:url.trim(),source:result.source});setStep("form");
    } catch (e: any) {setErrMsg(e.message||"Erreur inconnue");setStep("error");}
  };
  const add=()=>{
    if (!edited) return;
    onImport({...edited,occupant1Age:edited.occupant1Age||78,occupant1Sexe:edited.occupant1Sexe||"F",taxeFonciere:edited.taxeFonciere||400,chargesCopro:edited.chargesCopro||800,autresCharges:0,ventilationCharges:0.33,majorationRenteLiberation:0.30,tauxInflation:0.03,tauxCroissanceTF:0.04,loyerMensuelManuel:null,datePublication:new Date().toISOString().slice(0,10),note:edited.note||`Importé via ${edited.source}`,priceHistory:[{date:new Date().toISOString().slice(0,10),bouquet:edited.bouquet,rente:edited.rente}]});
    onClose();
  };
  const upd=(k: string,v: any)=>setEdited((p: any)=>({...p,[k]:v}));
  const field=(label: string,key: string,type="number")=>(
    <div style={{display:"flex",flexDirection:"column",gap:4}}>
      <label style={{fontSize:10,color:C.text3,textTransform:"uppercase"}}>{label}</label>
      <input type={type} value={edited?.[key]??""} onChange={e=>upd(key,type==="number"?(e.target.value===""?"":+e.target.value):e.target.value)} style={{background:C.surface,border:`1px solid ${C.border2}`,borderRadius:8,color:C.text,padding:"10px 14px",fontSize:13,width:"100%"}}/>
    </div>
  );
  const reset=()=>{setStep("input");setUrl("");setErrMsg("");setParsed(null);setEdited(null);};
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.92)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:200}} onClick={onClose}>
        ...
        <div style={{padding:"16px 20px 28px"}}>
          ...
          {step==="input"&&(
            <div>
              ...
              <input value={url} onChange={e=>setUrl(e.target.value)} onKeyDown={e=>e.key==="Enter"&&analyze()} placeholder="https://www.seloger.com/annonces/..." style={{background:C.card,border:`1px solid ${C.border2}`,borderRadius:10,color:C.text,padding:"13px 16px",fontSize:14,width:"100%",marginBottom:12}}/>
              ...
              <button onClick={analyze} disabled={!url.trim()} style={{background:C.orange,color:C.white,border:"none",borderRadius:12,padding:"14px 20px",cursor:"pointer",fontSize:15,fontWeight:700,width:"100%",opacity:url.trim()?1:0.4}}>Analyser →</button>
            </div>
          )}
          {step==="loading"&&<div style={{textAlign:"center",padding:"40px 0"}}><div style={{fontSize:36,marginBottom:14}}>⏳</div><div style={{fontSize:15,color:C.text2,fontWeight:600}}>Analyse en cours…</div><div style={{fontSize:11,color:C.text3,marginTop:8}}>Firecrawl scrape l'annonce · Extraction des données</div></div>}
          {step==="error"&&(
            <div>
              <div style={{background:`${C.red}15`,border:`1px solid ${C.red}30`,borderRadius:12,padding:16,marginBottom:16}}><div style={{fontSize:13,fontWeight:700,color:C.red,marginBottom:4}}>Erreur d'import</div><div style={{fontSize:11,color:C.text3}}>{errMsg}</div></div>
              <div style={{display:"flex",gap:10}}>
                <button onClick={()=>{setStep("form");setEdited({url,source:"Manuel"});}} style={{flex:1,background:C.card,color:C.text2,border:`1px solid ${C.border2}`,borderRadius:10,padding:"12px",cursor:"pointer",fontSize:13,fontWeight:600}}>Saisie manuelle</button>
          ...
          {step==="form"&&(
            <div>
              {parsed&&<div style={{background:`${C.green}12`,border:`1px solid ${C.green}25`,borderRadius:10,padding:12,marginBottom:16}}><div style={{fontSize:12,color:C.green,fontWeight:700}}>✅ {parsed.source} — {Math.round((parsed.confidence||0)*100)}% des champs extraits</div><div style={{fontSize:10,color:C.text3,marginTop:3}}>Complète les champs manquants ci-dessous</div></div>}
              ...
              <div style={{display:"flex",gap:10}}>
                <button onClick={add} style={{flex:1,background:C.orange,color:C.white,border:"none",borderRadius:12,padding:"14px",cursor:"pointer",fontSize:14,fontWeight:700}}>Ajouter au portefeuille</button>
    ...
    <div style={{background:C.card,borderRadius:16,overflow:"hidden",border:`1px solid ${C.border}`,cursor:"pointer",transition:"all .18s"}}
      ...
      <div style={{padding:16}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:6,marginBottom:12}}>
          {[["Bouquet",fmt(offre.bouquet)],["Rente",offre.rente?fmt(offre.rente)+"/m":"—"],["Durée",fmtYrs(duree)]].map(([k,v])=>(
        ...
        <div style={{display:"flex",gap:8}}>
          <button onClick={e=>{e.stopPropagation();onDetail(offre);}} style={{flex:1,background:`${C.orange}15`,color:C.orange,border:`1px solid ${C.orange}25`,borderRadius:10,padding:"9px",fontSize:12,cursor:"pointer",fontWeight:600}}>Détail complet</button>
          {offre.url&&<a href={offre.url} target="_blank" rel="noreferrer" onClick={e=>e.stopPropagation()} style={{background:C.surface,color:C.text3,border:`1px solid ${C.border}`,borderRadius:10,padding:"9px 12px",fontSize:12,fontWeight:600,textDecoration:"none"}}>🔗</a>}
          <button onClick={e=>{e.stopPropagation();onDelete(offre.id);}} style={{background:C.surface,color:C.text3,border:`1px solid ${C.border}`,borderRadius:10,padding:"9px 10px",fontSize:11,cursor:"pointer"}}>✕</button>
        </div>
      </div>
    </div>
  );
...
ViagerScan() {
  ...
  const delOffre = (id: any) => setOffres(p => p.filter((o: any) => o.id !== id));
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "-apple-system,'SF Pro Display','Inter',system-ui,sans-serif" }}>
      <div style={{ background: C.surface, borderBottom: `1px solid ${C.border}`, padding: `0 ${mobile ? 14 : 24}px`, position: "sticky", top: 0, zIndex: 50 }}>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: `16px ${mobile ? 12 : 20}px` }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 18 }}>
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Rechercher une ville…"
              style={{ background: C.card, border: `1px solid ${C.border2}`, borderRadius: 8, color: C.text, padding: "9px 14px", fontSize: 13, flex: 1, minWidth: 120 }} />
        ...
        <div style={{ marginTop: 32, padding: "14px 18px", background: C.card, borderRadius: 10, fontSize: 10, color: C.text3, border: `1px solid ${C.border}`, display: "flex", gap: 16, flexWrap: "wrap" }}>
          ...
          <span><strong style={{ color: C.text3 }}>Neon DB</strong> = Annonces sauvegardées automatiquement</span>
        </div>
      </div>
